...
def find_eligible_teacher(exercise_id, teachers_load, average_load):
    ...
    app.logger.info(
        f"Finding eligible teacher for exercise {exercise_id} with average load {average_load}"
    )

    # First, try to find a teacher who has previously worked on this exercise and isn't overloaded
    previous_teachers = (
        get_db().collection("tickets").where("exerciseId", "==", exercise_id).stream()
    )
...
def _obtener_todos_los_profesores_ids():
    teachers = (
        get_db().collection("teachers").stream()
    )  # Asume que 'db' es tu objeto de base de datos de Firestore
...
def get_teacher_loads():
    ...
    tickets = get_db().collection("tickets").stream()
